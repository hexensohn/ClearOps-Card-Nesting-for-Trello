(function () {
  ...
  function normalizeBoardIndex(index) {
    const parentCardIds = Array.isArray(index && index.parentCardIds)
      ? Array.from(new Set(index.parentCardIds.filter(Boolean)))
      : [];
  ...
  async function getTrelloLegacyStore(targetT = getIframeContext()) {
    const store = await targetT.get("board", "shared", LEGACY_STORAGE_KEY);
    return normalizeStore(store);
  }

  async function getTrelloBoardIndex(targetT = getIframeContext()) {
    const index = await targetT.get("board", "shared", BOARD_INDEX_KEY);
    return normalizeBoardIndex(index);
  }

  async function getTrelloParentEntryById(parentCardId, targetT = getIframeContext()) {
    const entry = await targetT.get(parentCardId, "shared", PARENT_STORAGE_KEY);
    return normalizeParentEntry(parentCardId, entry);
  }

  async function readTrelloBackedStore(targetT = getIframeContext()) {
    const legacyStore = await getTrelloLegacyStore(targetT);
    if (Object.keys(legacyStore.parentsById || {}).length) {
      return legacyStore;
    }

    let boardIndex = await getTrelloBoardIndex(targetT);
    if (!boardIndex.parentCardIds.length) {
      const board = await targetT.board("id");
      const boardCards = await getBoardCards(board.id).catch(function () {
        return [];
      });
      const scannedEntries = await Promise.all(
        boardCards.map(function (boardCard) {
          return getTrelloParentEntryById(boardCard.id, targetT).catch(function () {
            return null;
          });
        })
      );

      boardIndex = normalizeBoardIndex({
        parentCardIds: scannedEntries
          .filter(function (entry) {
            return entry && entry.isParent && entry.parentCardId;
          })
          .map(function (entry) {
            return entry.parentCardId;
          })
      });
    }

    const parentEntries = await Promise.all(
      boardIndex.parentCardIds.map(function (parentCardId) {
        return getTrelloParentEntryById(parentCardId, targetT);
      })
    );

    return normalizeStore({
      parentsById: parentEntries.reduce(function (acc, entry) {
        if (entry && entry.parentCardId && entry.isParent) {
          acc[entry.parentCardId] = entry;
        }
  ...
  async function getBackendStoreForBoard(boardId) {
    const result = await backend(`/api/boards/${encodeURIComponent(boardId)}/store`);
    return {
      found: Boolean(result && result.found),
      store: normalizeStore(result && result.store)
    };
  }

  async function setBackendStoreForBoard(boardId, store) {
    const result = await backend(`/api/boards/${encodeURIComponent(boardId)}/store`, {
      method: "PUT",
      body: {
    ...
    return normalizeStore(result && result.store);
  }

  async function ensureStorageReady(targetT = getIframeContext()) {
    const board = await targetT.board("id");
    const boardId = (board && board.id) || "unknown-board";

    if (storageReadyBoards.has(boardId)) {
      return;
    }

    const backendStore = await getBackendStoreForBoard(boardId);
    if (!backendStore.found) {
      const trelloStore = await readTrelloBackedStore(targetT);
      if (Object.keys(trelloStore.parentsById).length) {
        await setBackendStoreForBoard(boardId, trelloStore);
      }
    }

    storageReadyBoards.add(boardId);
  }
  ...
  async function setStore(nextStore, targetT = getIframeContext()) {
    const board = await targetT.board("id");
    return setBackendStoreForBoard(board.id, nextStore);
  }
  ...
  async function updateStore(mutator, targetT = getIframeContext()) {
    ...
    return setStore(result || draft, targetT);
  }

  async function getParentEntryById(parentCardId, targetT = getIframeContext()) {
    const store = await getStore(targetT);
  ...
  function normalizeParentEntry(parentCardId, entry) {
    return {
      parentCardId,
      isParent: Boolean(entry && entry.isParent),
  ...
  function normalizeStoredChildItem(childItem) {
    return {
      id: (childItem && childItem.id) || (childItem && childItem.sourceCardId) || makeId(),
  ...
  function buildStoredChildItem(childInput) {
    return normalizeStoredChildItem({
      id: childInput.sourceCardId || makeId(),
      title: (childInput.title || "").trim(),
      description: childInput.sourceCardId ? "" : (childInput.description || "").trim(),
      sourceCardId: childInput.sourceCardId || ""
    });
  }

  function compactStoredChildItemForStorage(childItem) {
    const normalizedChild = normalizeStoredChildItem(childItem);

    if (normalizedChild.sourceCardId) {
      normalizedChild.id = normalizedChild.sourceCardId;
  ...
  function compactParentEntryForStorage(parentCardId, entry) {
    const normalizedEntry = normalizeParentEntry(parentCardId, {
      ...(entry || {}),
      isParent: true
    });
    ...
    return {
      ...normalizedEntry,
      isParent: true,
      childItems: normalizedEntry.childItems.map(compactStoredChildItemForStorage)
    };
  }
  ...
  function isPluginDataLimitError(error) {
    return Boolean(error && error.message && error.message.includes("PluginData length"));
  }

  function rethrowStorageLimitError(error, fallbackMessage) {
    if (isPluginDataLimitError(error)) {
      throw new Error(
        fallbackMessage ||
          "Trello Power-Up storage was exceeded for this parent card. Extract some nested cards or move nested-card storage to a backend database."
      );
    }
  ...
  async function setParentCard({ label }) {
    ...
    const parentEntry = await getParentEntryById(ctx.card.id);
    parentEntry.isParent = true;
    parentEntry.label = (label || "").trim() || parentEntry.label || ctx.card.name;

    try {
      await setParentEntryById(ctx.card.id, parentEntry);
    } catch (error) {
      rethrowStorageLimitError(error, "Unable to save this parent because Trello Power-Up storage is full for that card.");
    }
...
async function addChildItemToParent(parentCardId, childInput) {
  ...
  const parentEntry = await getParentEntryById(parentCardId);
  if (!parentEntry.isParent) {
  ...
  try {
    await setParentEntryById(parentCardId, parentEntry);
  } catch (error) {
    rethrowStorageLimitError(
      error,
      "Unable to nest this card because Trello Power-Up storage is full for the selected parent. Extract some nested cards or move the nested-card payload to a backend."
    );
  }
...
async function bulkStoreCardsInCurrentParent(cardIds, options = {}) {
  ...
  for (const selectedCard of selectedCards) {
    ...
    if (isParentCard(ctx.store, selectedCard.id)) continue;
    if (nestedSourceIds.has(selectedCard.id)) continue;
    if (findEmbeddedSource(ctx.store, selectedCard.id)) continue;
  ...
  try {
    ...
    rethrowStorageLimitError(
      error,
      "Unable to add these cards because Trello Power-Up storage is full for this parent. Extract some nested cards or move the nested-card payload to a backend."
    );
  }
